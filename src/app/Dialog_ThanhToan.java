...
public class Dialog_ThanhToan extends JDialog implements ActionListener {
	...
	public Dialog_ThanhToan(String maPhong) {
		...
		for(ChiTietDichVu ctdv : ctdv_dao.getChiTietDichVuTheoMaHD(lbl_MaHoaDon_1.getText().trim())) {
			...
			Object[] rowSanPham = {j++,spdv.getTenSanPham(), ctdv.getSoLuong(), ctdv.getGia(), spdv.getDonViTinh(), ctdv.getSoLuong() * ctdv.getGia()};
		...
		lbl_TongThanhTien_1 = new JLabel(f.format((tongTienDichVu+tongTienPhong) + 0.1*(tongTienDichVu+tongTienPhong)) + " VNĐ");
		...
		lbl_ChiecKhau.setBounds(360, 330, 90, 25);
...
//		txtMaGiamGia.setText("");
		...
		DocumentListener documentListener = new DocumentListener() {
			...
			public void updateTxtTienThua() {
				try {
					...
					// Thực hiện công thức
					double tienThua = tienNhan - Double.parseDouble(lbl_TongThanhTien_1.getText().replaceAll(" VNĐ", ""));
	...
	private void thanhToan() {
		if(txtTienNhan.getText().trim().equals("") || txtMaGiamGia.isEditable()) {
			JOptionPane.showMessageDialog(null, "Bạn chưa nhập tiền nhận từ khách hàng hoặc chưa kiểm tra mã khuyến mãi!!");
		}else {
			if(JOptionPane.showConfirmDialog(null, "Bạn có chắc chắn muốn thanh toán hóa đơn này", "Thông báo", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_OPTION) {
				//Update Phòng
				...
				if(txtMaGiamGia.getText().equals(""))
				{
					...
					HoaDonDatPhong hd_update2 = new HoaDonDatPhong(maHD, kh_update, nv, ngayLap, trangThai_ThanhToan, km2, tienKhachDua);
				...
				for (int row = 0; row < tblThanhToan.getRowCount(); row++) {		
					...
					ChiTietHoaDon cthd_Item = new ChiTietHoaDon(hd, ph, ngayGioNhanPhong, ngayGioTraPhong, thoiGianHat);
