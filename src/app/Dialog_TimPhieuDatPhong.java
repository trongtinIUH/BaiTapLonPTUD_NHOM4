...
public class Dialog_TimPhieuDatPhong extends JDialog implements ActionListener, MouseListener {
	...
	private String col[] = { "Mã PDP", "Phòng", "Mã NV", "Mã KH", "   Ngày Giờ Đặt   ", "   Ngày Giờ Nhận   ",
			"Số Người", "Hình Thức" };
	...
	public Dialog_TimPhieuDatPhong() {
		...
		txtMaPDP.setBounds(115, 6, 220, 30);
		...
		lblTrangThai.setBounds(345, 5, 100, 30);
		...
		comboBox_TrangThai
				.setModel(new DefaultComboBoxModel<String>(new String[] { "Chưa Thanh Toán", "Đã Thanh Toán" }));
	...
	public void loadData() {
		...
		for (PhieuDatPhong x : allPhieuDatPhong) {
			...
			p = pdp.getPhong();
			Object[] row = { x.getMaPhieu(), p.getMaPhong(), x.getNhanVien().getMaNhanVien(),
					x.getKhachHang().getMaKhachHang(), ngayGioDat, ngayGioNhan, x.getSoNguoiHat(), hinhthuc };
			model.addRow(row);
		}
		Canh_Deu_Bang();
	}
	...
	public void tim() {
		...
		if (btnTimKiem.getText().equals("Tìm kiếm")) {
			if (!maPhieu.isEmpty() && !sdt.isEmpty()) {
				...
				return;
			} else if (maPhieu.isEmpty() && sdt.isEmpty()) {
				JOptionPane.showMessageDialog(null, "Bạn chưa nhập thông tin tìm kiếm!");
				return;
			} else if (!maPhieu.isEmpty()) {
				// Tìm kiếm theo mã phiếu

				if (pdp != null) {
					btnTimKiem.setText("Hủy tìm");
					clearTable();
					Object[] row = { pdp.getMaPhieu(), pdp.getPhong().getMaPhong(), pdp.getNhanVien().getMaNhanVien(),
							pdp.getKhachHang().getMaKhachHang(), pdp.getNgayGioDatPhong(), pdp.getNgayGioNhanPhong(),
							pdp.getSoNguoiHat(), hinhthuc };
					model.addRow(row);
					Canh_Deu_Bang();
				} else {
				...
				if (kh != null) {
					ArrayList<PhieuDatPhong> dsPDP = pdp_dao.getDSPhieuDatPhongTheoMaKH(makh);
					if (dsPDP.size() != 0) {
						btnTimKiem.setText("Hủy tìm");
						clearTable();
						for (PhieuDatPhong pd : dsPDP) {
							if (!pd.getNgayGioDatPhong().isEqual(pd.getNgayGioNhanPhong())) {
								hinhthuc = "Đặt trước";
							} else
								hinhthuc = "Đặt trực tiếp";
							Object[] row = { pd.getMaPhieu(), pd.getPhong().getMaPhong(),
									pd.getNhanVien().getMaNhanVien(), pd.getKhachHang().getMaKhachHang(),
									pd.getNgayGioDatPhong(), pd.getNgayGioNhanPhong(), pd.getSoNguoiHat(), hinhthuc };
							model.addRow(row);
						}
						Canh_Deu_Bang();
					} else {
